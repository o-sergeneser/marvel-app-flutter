...
void main() {
  group('fetchData', () {
    test(
        'returns characters info of Marvel string when http response is successful',
        () async {
                ...
                "urls": [
                  {
                    "type": "detail",
                    "url":
                        "http://marvel.com/characters/74/3-d_man?utm_campaign=apiRef&utm_source=c9250343fb2d3eeac3d2754813289a2c"
                  },
                  {
                    "type": "wiki",
                    "url":
                        "http://marvel.com/universe/3-D_Man_(Chandler)?utm_campaign=apiRef&utm_source=c9250343fb2d3eeac3d2754813289a2c"
                  },
                  {
                    "type": "comiclink",
                    "url":
                        "http://marvel.com/comics/characters/1011334/3-d_man?utm_campaign=apiRef&utm_source=c9250343fb2d3eeac3d2754813289a2c"
                  }
                ]
              }
            ]
      ...
      expect(
          await WebServices.fetchData(mockHTTPClient, "characters",
              extraParams: "&limit=1&offset=0"),
          isA<Map>());
    });
    ...
    test('return error message when http response is unsuccessful', () async {
      ...
      final mockHTTPClient = MockClient((request) async {
        ...
        return Response(jsonEncode(response), 409);
      });
      expect(
          await WebServices.fetchData(mockHTTPClient, "characters",
              extraParams: "&limit=101&offset=0"),
          'You may not request more than 100 items.');
    });
  });
}
